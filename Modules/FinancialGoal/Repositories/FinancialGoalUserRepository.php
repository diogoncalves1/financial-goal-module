...
namespace Modules\FinancialGoal\Classes\Repositories;
...
use Modules\FinancialGoal\Models\FinancialGoalUser;
...
class FinancialGoalUserRepository
{
    ...
    public function __construct(FinancialGoalRepository $financialGoalRepository /*, FriendshipRepository $FriendshipRepository */, SharedRoleRepository $sharedRoleRepository)
    {
    ...
    public function inviteUser(Request $request, string $id)
    {
        try {
            return DB::transaction(function () use ($request, $id) {
                ...
                $sharedRole = $this->financialGoalRepository->userSharedRole($financialGoal, 2);
                ...
                $userRelation = FinancialGoalUser::create($input);
    ...
    public function acceptInvite(Request $request, string $id)
    {
        try {
            return DB::transaction(function () use ($request, $id) {
                ...
                $userRelation = FinancialGoalUser::query()->user(3)->account($id)->status("accepted")->first();
    ...
    public function destroyInvite(Request $request, string $id)
    {
        try {
            return DB::transaction(function () use ($request, $id) {
                $account = $this->financialGoalRepository->show($id);

                $user = $request->user();

                $sharedRole = $this->financialGoalRepository->userSharedRole($account, 2);
    ...
    public function revokeInvite(Request $request, string $id)
    {
        try {
            return DB::transaction(function () use ($request, $id) {
                ...
                $userRelation = FinancialGoalUser::query()->user(1)->account($id)->status("revoked")->first();
    ...
    public function revokeUser(Request $request, string $id)
    {
        try {
            return DB::transaction(function () use ($request, $id) {
                ...
                $sharedRoleInvite = $this->financialGoalRepository->userSharedRole($account, $request->get('user_id'));
    ...
    public function updateUserRole(Request $request, string $id)
    {
        try {
            return DB::transaction(function () use ($request, $id) {
                ...
                $sharedRoleUserUpdate = $this->financialGoalRepository->userSharedRole($account, $request->get('user_id'));
    ...
    private function exceededDeclines(string $receiverId, string $accountId, int $maxDeclines, int $days)
    {
        ...
        return FinancialGoalUser::query()->user($receiverId)->account($accountId)->status('revoked')->where('invited_at', '>=', $limitDate)->count() >= $maxDeclines;
    }
    private function hasPendingRequest(string $receiverId, string $accountId)
    {
        return FinancialGoalUser::query()->user($receiverId)->account($accountId)->status('pending')->exists();
    }
    private function hasRelation(string $userId, string $accountId)
    {
        return FinancialGoalUser::query()->user($userId)->account($accountId)->status('accepted')->exists();
    }
    private function destroy(string $userId, string $accountId, string $status)
    {
        return FinancialGoalUser::query()->user($userId)->account($accountId)->status($status)->delete();
    }
    private function update(string $userId, string $accountId, string $status, array $input)
    {
        return FinancialGoalUser::query()->user($userId)->account($accountId)->status($status)->update($input);
    }
}
